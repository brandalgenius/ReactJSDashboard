...
import MailAdd from './MailAddInput.component'
import MailModalComponent from './MailAddModal.component'
import { Form, notification, Tooltip, Icon, Button } from 'antd'
import { WrapperMailAdd } from './MailAdd.style'
...
const MailAddContainer = props => {
  ...
  const fetchMail = async () => {
    await adminMailService
      .SEARCH_MAIL_SETTING(1, stateListing.size, {}, stateUser.token)
      .then(search => {
        dispatch({
          type: 'listing/SUBMIT',
          payload: {
    ...
    await form.validateFields(async (err, values) => {
      if (!err) {
        ...
        const bodyPost = {
          ...
          industryList: values.industries,
          regionList: values.regions,
          biddingMethodList: values.biddingMethods,
        }
        ...
        await adminMailService
          .ADD_MAIL_SETTING(1, stateListing.size, bodyPost, stateUser.token)
          .then(() => {
            fetchMail()
            onReset()
...
const WrapperMailAddContainer = Form.create({
  name: 'MailAddContainer',
})(MailAddContainer)

export default WrapperMailAddContainer
